...
def write_data(path,file,data):
    ...
    with open(path + "/" + file, "w") as f:
        for k, per_data in enumerate(data):
            out = None
            try:  # 针对ts只有1维的情况
                for i, d in enumerate(per_data):
                    if out is None:
                        out = str(d)
                    else:
                        out += str(d)
...
def save_hdf5(args):
    ...
    for i in progressbar.progressbar(range(n_data), redirect_stdout=True):
        ...
        attitude = np.loadtxt(osp.join(datapath, "atttitude.txt"), delimiter=",")
        if blackbird:
            targ_rpm = np.loadtxt(osp.join(datapath, "targ_rpm.txt"), delimiter=",")
            meas_rpm = np.loadtxt(osp.join(datapath, "meas_rpm.txt"), delimiter=",")

        goal = np.loadtxt(osp.join(datapath, "goal_ts.txt"), delimiter=",")
        goal_index = []
        try:
            for goal_t in goal:
                goal_index.append(np.where(image_ts == goal_t)[0][0])
        except:
            goal_index.append(np.where(image_ts == goal)[0][0])
        goal_index.append(image_ts.shape[0] - 1)

        for index_num in range(len(goal_index)-1):

            print("index_num: ",index_num)

            image_ts = np.loadtxt(osp.join(datapath, "my_timestamps_p.txt"))[goal_index[index_num]:goal_index[index_num+1]]
            imu_meas = np.loadtxt(osp.join(datapath, "imu_measurements.txt"), delimiter=",")[goal_index[index_num]:goal_index[index_num+1]]
            gt_states = np.loadtxt(osp.join(datapath, "evolving_state.txt"), delimiter=",")[goal_index[index_num]:goal_index[index_num+1]]
            attitude = np.loadtxt(osp.join(datapath, "atttitude.txt"), delimiter=",")[goal_index[index_num]:goal_index[index_num+1]]
            if blackbird:
                targ_rpm = np.loadtxt(osp.join(datapath, "targ_rpm.txt"), delimiter=",")[goal_index[index_num]:goal_index[index_num+1]]
                meas_rpm = np.loadtxt(osp.join(datapath, "meas_rpm.txt"), delimiter=",")[goal_index[index_num]:goal_index[index_num+1]]
            ...
            attitude_idx = np.searchsorted(attitude[:, 0], start_t)
            if blackbird:
            ...
            if index_num == 0:
                ba = acc - acc
                ba_mean = np.average(ba, axis=0)
                ba_dev = np.abs(ba - ba_mean)

                bg = gyr - gyr
                bg_mean = np.average(bg, axis=0)
                bg_dev = np.abs(bg - bg_mean)

                ind_ba = max(int(np.argwhere(ba_dev[:, 0] < 0.1)[0]),
                             int(np.argwhere(ba_dev[:, 1] < 0.1)[0]),
                             int(np.argwhere(ba_dev[:, 2] < 0.1)[0]))
                ind_bg = max(int(np.argwhere(bg_dev[:, 0] < 0.1)[0]),
                             int(np.argwhere(bg_dev[:, 1] < 0.1)[0]),
                             int(np.argwhere(bg_dev[:, 2] < 0.1)[0]))
                ind_st = max(ind_ba, ind_bg)
                print("ind_st: ", ind_st)
                image_ts = np.loadtxt(osp.join(datapath, "my_timestamps_p.txt"))[
                           goal_index[index_num] + ind_st:goal_index[index_num + 1]]
                imu_meas = np.loadtxt(osp.join(datapath, "imu_measurements.txt"), delimiter=",")[
                           goal_index[index_num] + ind_st:goal_index[index_num + 1]]
                gt_states = np.loadtxt(osp.join(datapath, "evolving_state.txt"), delimiter=",")[
                             goal_index[index_num] + ind_st:goal_index[index_num + 1]]
            ...
            attitude_data[0,:] = attitude[attitude_idx,1:]
            if blackbird:
            ...
            for i in progressbar.progressbar(range(1, N)):
                ...
                if has_gt == 1:
                    ...
                    attitude_idx = np.searchsorted(attitude[:, 0], imu_data[i, 0])
                    if blackbird:
                    ...
                    attitude_data[i,:] = attitude[attitude_idx,1:]
            ...
            outdir = osp.join("/home/jiangcx/桌面/TLIO/DL_IMU/12.29_xsens/hdf5", n + '(' + str(index_num) + ')')
            ...
            with h5py.File(osp.join(outdir, "data.hdf5"), "w") as f:
                ...
                attitude_s = f.create_dataset("attitude", data=attitude_data)
                if blackbird:
            ...
            write_data(outdir, "my_timestamps_p.txt", image_ts)
            write_data(outdir, "imu_measurements.txt", imu_meas)
            write_data(outdir, "evolving_state.txt", gt_states)
            write_data(outdir, "attitude.txt", attitude)
            write_data(outdir, "targ_rpm.txt", targ_rpm)
            write_data(outdir, "meas_rpm.txt", meas_rpm)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="/home/jiangcx/桌面/TLIO/DL_IMU/12.29_xsens")
    parser.add_argument(
        "--data_dir", type=str, default="/home/jiangcx/桌面/TLIO/DL_IMU/12.29_xsens"
    )
    parser.add_argument(
        "--data_list",
        type=str,
        default="/home/jiangcx/桌面/TLIO/DL_IMU/12.29_xsens/xsens_12.29_list.txt",
    )
