...
def loss_distribution_diag_R3(pred, pred_cov, targ):
    ...
    # print('=========================================')
    # print("1 : ***********************************************")
    # print(torch.mean((pred - targ).pow(2),dim=0))
    # print("2 : ***********************************************")
    # print(torch.mean(2 * torch.exp(2 * pred_cov),dim=0))
    # print("3 : ***********************************************")
    print(torch.mean(pred_cov,dim=0))
...
def loss_mse_so3_q(pred_q, targ_q):
    ...
    rela_q_normalized[rela_q_normalized[:, 0] < 0] = (-1) * rela_q_normalized[rela_q_normalized[:, 0] < 0]
    
    e = q_log_torch(rela_q_normalized)
    loss = e.pow(2)
    return loss

def loss_mse_SO3(pred_q, targ_q):
    pred_q_normalized = F.normalize(pred_q, dim=len(pred_q.shape) - 1)
    targ_q_normalized = F.normalize(targ_q, dim=len(pred_q.shape) - 1)

    rela_q_SO3 = pred_q_normalized - targ_q_normalized
    # loss_fuc = torch.nn.SmoothL1Loss(reduction='none')
    loss_SO3 = rela_q_SO3.pow(2)
    return loss_SO3
"""
